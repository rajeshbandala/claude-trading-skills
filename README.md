...
| **Robinhood Portfolio Manager** (`robinhood-portfolio-manager`) | Comprehensive portfolio analysis using Robinhood MCP Server integration to fetch holdings and positions, then analyze asset allocation, risk metrics, individual stock positions, diversification, and generate rebalancing recommendations. Optionally place orders with a strict confirm-first, one-order-at-a-time guardrail. | `robinhood` **required** | production |
| **Trading Desk Operator** (`trading-desk-operator`) | Run a disciplined, risk-first equities trading desk over a Robinhood (MCP) book and watchlist, posting a numbers-first briefing to Slack each cycle. Reads the watchlist and book from all accounts and discovers fresh candidates from news + technicals (gated to S&P 500 constituents), but places real orders only on the agentic-allowed account through a confirm-first gate (optional bounded auto-execution). | `robinhood` **required**, `slack` optional, `websearch` **required** | production |
