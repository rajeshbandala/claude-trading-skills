...
| **[Trading Desk Operator]({{ '/en/skills/trading-desk-operator/' | relative_url }})** | Risk-first equities desk over a Robinhood (MCP) book + watchlist with a Slack briefing each cycle. Reads watchlist/book from all accounts but places real bracket orders only on the agentic-allowed account through a confirm-first gate (optional bounded auto-execution). Regime → news → technicals → ranked setups → risk sizing → position management. | <span class="badge badge-api">Robinhood Required</span> <span class="badge badge-optional">Slack Optional</span> |
