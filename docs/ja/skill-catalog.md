...
| **[Robinhood Portfolio Manager]({{ '/ja/skills/robinhood-portfolio-manager/' | relative_url }})** | Robinhood MCP Serverで保有データを取得。資産配分、分散、リスク指標、HOLD/ADD/TRIM/SELL推奨、リバランス計画に加え、確認優先（1注文ずつ）の任意執行をサポート | <span class="badge badge-api">Robinhood必須</span> |
...
| **[Trading Desk Operator]({{ '/ja/skills/trading-desk-operator/' | relative_url }})** | Robinhood（MCP）の保有・ウォッチリスト上でリスクファーストの株式トレーディングデスクを運用し、各サイクルでSlackにブリーフィングを投稿。全口座からウォッチリスト・保有を読み取り、ニュース＋テクニカルから新規候補も発掘（S&P 500構成銘柄に限定）。実際のブラケット注文はagentic許可口座でのみ確認優先ゲートを通じて発注（任意の制限付き自動執行）。レジーム→ニュース→テクニカル→発掘→ランク付けセットアップ→リスクサイジング→ポジション管理。 | <span class="badge badge-api">Robinhood必須</span> <span class="badge badge-optional">Slack任意</span> |
