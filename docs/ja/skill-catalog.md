...
| **[Trading Desk Operator]({{ '/ja/skills/trading-desk-operator/' | relative_url }})** | Robinhood（MCP）の保有・ウォッチリスト上でリスクファーストの株式トレーディングデスクを運用し、各サイクルでSlackにブリーフィングを投稿。全口座からウォッチリスト・保有を読み取るが、実際のブラケット注文はagentic許可口座でのみ確認優先ゲートを通じて発注（任意の制限付き自動執行）。レジーム→ニュース→テクニカル→ランク付けセットアップ→リスクサイジング→ポジション管理。 | <span class="badge badge-api">Robinhood必須</span> <span class="badge badge-optional">Slack任意</span> |
