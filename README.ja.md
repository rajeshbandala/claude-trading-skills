...
| **Trading Desk Operator** (`trading-desk-operator`) | Run a disciplined, risk-first equities trading desk over a Robinhood (MCP) book and watchlist, posting a numbers-first briefing to Slack each cycle. Reads the watchlist and book from all accounts but places real orders only on the agentic-allowed account through a confirm-first gate (optional bounded auto-execution). | `robinhood` **required**, `slack` optional, `websearch` **required** | production |
